...
| `match_engine` | Phase-2a engine, now tactics-, consistency-, and condition-aware (`TACTICS_MODEL.md` §6-§7, `MATCH_MODEL.md` §17/§13, T6-T9): `play_match(world, home, away, rng, consistency_rng, k: &Knobs, conditions: &BTreeMap<PlayerId, f64>)` (`MatchOutcome { home_goals, away_goals, stream }` plus the §12 boundary fields `injuries`/`cards`/`ratings`/`minutes` — `minutes` a flat 90 per starter until T10/T11/T12, the rest empty until their models land; `consistency_rng` is its own stream, `k` lets callers pin `consistency_sigma_max` independent of the production default — the T5/T6 identity tests' seam; `conditions` is a pre-computed per-player map — `GameState::condition` output, since the engine itself has no `GameState` — absent-from-map defaulting to `1.0`, the identity), `lineup_strength`, `ai_pick_lineup` (XI selection only, always neutral tactics), `ai_pick_tactics`/`AiTacticKnobs` (the §7 policy — implemented, tested, but gated), `ai_pick_lineup_vs` (the real call-site: XI + tactics, tactics applied only when `AI_TACTICS_ENABLED` — currently `false`, pending `TACTICS_MODEL.md` §9 item 6's open design question — is `true`; every real AI match runs `Tactics::neutral()` today). Submodules: `zone` (five-zone state + role→zone presence table), `knobs` (the fitted `Knobs` table, now including `consistency_sigma_max`/`consistency_mult_min`/`consistency_mult_max`, T8), `contest` (attribute→contest maps, the logistic resolver, fatigue — `fatigue_mult` takes a per-side `press_mult` and a `condition` scaling its minute-0 starting point, T9), `tactics` (§3's per-side `SideEffects` resolution — pure, RNG-free, computed once per match; `b_pass_delta_by_zone` and `Pressing::High`'s zone-profiled bias are the T7-addendum fixes, §5), `resolve` (the possession loop; `select_action`/`step`/`take_shot`/`simulate` all read `SideEffects` alongside `Knobs`; `build_xi` draws each XI's Consistency multipliers and attaches each `XiPlayer`'s `condition` from the caller's map, T8/T9), `stream` (`MatchEvent` schema + commentary rendering), `golden` (`#[cfg(test)]`: the batch-3 T5 pinned Phase-2a baseline every 2e identity invariant asserts against), `calibrate` (`StreamTelemetry`'s per-zone pass-completion and turnover-won-by-zone cuts, `run_head_to_head` — the §7 triangle harness; stays at the condition identity, §13's status note) |
