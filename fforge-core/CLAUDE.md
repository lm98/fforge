...
`AI_TACTICS_ENABLED` stays `false`, but on a **new** blocker (`TACTICS_MODEL.md` §9 item 7):
`Mentality::Attacking` beats `Balanced` 0.530/0.470 — the same lever-class mismatch,
uncorrected on that axis (two advance-class gains, one logit-class cost). The flag is
mechanically safe to flip (161/161 green with it on, all four pooled guards included), but
`ai_pick_tactics` sets Mentality from the strength gap, so flipping now would inject that
dominance into every AI match and move pooled goals/match 2.84 → 3.19.
