...
unchanged. `ai_pick_lineup` still emits `Tactics::neutral()` everywhere; the AI tactics
policy (§7) is T7's job.
...
| `match_engine` | Phase-2a engine, now tactics-aware (`TACTICS_MODEL.md` §6, T6): `play_match` (`MatchOutcome { home_goals, away_goals, stream }` plus the §12 boundary fields `injuries`/`cards`/`ratings`/`minutes` — `minutes` a flat 90 per starter until T10/T11/T12, the rest empty until their models land), `lineup_strength`, `ai_pick_lineup` (still emits `Tactics::neutral()` everywhere; T7 wires in `ai_pick_tactics`). Submodules: `zone` (five-zone state + role→zone presence table), `knobs` (the fitted `Knobs` table), `contest` (attribute→contest maps, the logistic resolver, fatigue — `fatigue_mult` takes a per-side `press_mult`), `tactics` (§3's per-side `SideEffects` resolution — pure, RNG-free, computed once per match), `resolve` (the possession loop; `select_action`/`step`/`take_shot`/`simulate` all read `SideEffects` alongside `Knobs`), `stream` (`MatchEvent` schema + commentary rendering), `golden` (`#[cfg(test)]`: the batch-3 T5 pinned Phase-2a baseline every 2e identity invariant asserts against) |
